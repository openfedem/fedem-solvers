...
contains
  ...
  !> of the global X-axis onto the plane defined by the given vector @a VZ.
  !>
  !> @callergraph
  !>
...
600 format(26X,'V1 =',1P3E13.5 / 26X,'V2 =',1P3E13.5 / 26X,'V3 =',1P3E13.5, &
  ...
  !>
  !> @param[in] VX X-axis of the element coordinate system
  !> @param[in] VZ Z-axis of the element coordinate system
  ...
  subroutine getShellStressTrans (VX, VZ, T, lpu, ierr)

    use KindModule       , only : dp, epsDiv0_p
    use manipMatrixModule, only : cross_product

    real(dp), intent(in)  :: VX(3), VZ(3)
    ...
    if (dot_product(V1,V1) <= epsDiv0_p*epsDiv0_p) then
       write(lpu,*) '*** getShellStressTrans: Invalid shell normal vector',VZ
       ierr = -1
       return
    end if
